...
export default {
	...
	callback(editor) {
		var changeNonPlainText = atom.config.get('shebang-set-grammar.changeNonPlainText');
		if (! changeNonPlainText) {
			if (editor.getGrammar() != atom.grammars.nullGrammar) {
				return;
			}
		}
		
		var bin_name = this.read_shebang(editor.lineTextForBufferRow(0));
		if (bin_name == null) {
			return;
		}
